...
; what version of Emacs are we running?
(setq xemacs (boundp 'xemacs-codename))
(setq emacs19 (string-match "^19" emacs-version))
(setq emacs20 (string-match "^20" emacs-version))


;; -- Patched/enhanced emacs files --------------------------------------------
(cond (emacs19 (load (concat utils-dir "/lisp/compile19.el")))
	  (emacs20 (load (concat utils-dir "/lisp/compile.el"))))
...
(if xemacs
	(progn
	  ...
	  (define-key global-map [control-up] 'scroll-down-one)
	  (define-key global-map [control-kp-up] 'scroll-down-one)
	  (define-key global-map [control-down] 'scroll-up-one)
	  (define-key global-map [control-kp-down] 'scroll-up-one)
	  ...
	  (define-key global-map [meta-kp-up] 'backward-list)
	  (define-key global-map [meta-down] 'forward-list)
	  (define-key global-map [meta-kp-down] 'forward-list))
...
(if xemacs
  ...
  (setq win32-quote-process-args t)	;xemacs equivalent
...
(define-key global-map [control-tab] 'bubble-buffer)
(define-key global-map [control-shift-tab] 'bubble-buffer-back)
;...replaces...
;(define-key global-map [control-tab] 'bury-buffer)
;(defun unbury-buffer()
;  (interactive)
;  (switch-to-buffer (nth (1- (length (buffer-list))) (buffer-list))))
;(define-key global-map [control-shift-tab] 'unbury-buffer)
...
(defun previous-window-all-frames ()
  (interactive) (other-window -1 t) (other-frame 0))
(define-key global-map [shift-f6] 'previous-window-all-frames)
...
(if xemacs (setq default-frame-alist default-frame-plist))
...
(or xemacs
	(progn
	  ...
	  (load "hilit19")
	  (load (concat utils-dir "/lisp/hilit.el")) ; some extra languages
...
%s\n\
%s$Id: //open/users/jhyde/home/lisp/XEmacs.el#1 $\n\
...
;; lisp to the server using 'gnudoit'.
(if (not xemacs)
    (progn
      (require 'gnuserv)
      (gnuserv-start)
