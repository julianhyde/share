...
: \\(error\\|warning\\) C[0-9]+:" 1 3)
    ...
    ;; Microsoft J++ (added by jhyde, 980114):
    ;;  Broadbase\ConnectEvent.java(16,9) : error J0056: Missing return type
    ;was:
	;("\n\\(\\([a-zA-Z]:\\)?[^:( \t\n-]+\\)[:(][ \t]*\\([0-9]+\\),\\([0-9]+\\)[:) \t]" 1 3 4)
    ("\\(\\([a-zA-Z]:\\)?[^:(\t\n]+\\)(\\([0-9]+\\),\\([0-9]+\\)) \
: \\(\\(fatal \\)?error\\|warning\\) J[0-9]+:" 1 3 4)

    ;; Microsoft JavaReg (added by jhyde, 990219):
    ;;  C:\WINNT\java\trustlib\dispatcherlib\summary.txt(1): Class summary...
    ("\\(\\([a-zA-Z]:\\)?[^:(\t\n]+\\)(\\([0-9]+\\)):" 1 3)
